...
def solution(m, musicinfos):
    ...
    album = {i[2]:i[3]for i in music}
    for i in music:
        playtime = (int(i[1].split(':')[1])-int(i[0].split(':')[1]))+(int((i[1].split(':')[0])) -int((i[0].split(':')[0]))) * 60      
        if playtime < len(i[3]):
            album[i[2]]=change(i[3][:playtime])
        else:
            
            a,b = divmod(playtime, len(i[3]))
            album[i[2]]=change(i[3]*a+i[3][:b])
    s={}
    for i,v in album.items():
        s[v.count(m)]=i
    return s[max(s)] if max(s) >0 else "(None)"
...
def change(song):
    song = song.replace('C#','c')
    song = song.replace('D#','d')
    song = song.replace('F#','F') ## 소문자 f 로 바꾸면 100프로
    song = song.replace('G#','g')
    song = song.replace('A#','a')
    return song
...
def solution(m, musicinfos):
    ...
    if len(answer) !=0:
        answer = sorted(answer, key=lambda x: (-x[1],[0]))
        return answer[0][2]
    return "(None)"
#효율성 97
```
